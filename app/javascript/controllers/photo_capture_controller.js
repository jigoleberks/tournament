...
export default class extends Controller {
  ...
  async _renderCameraDebug() {
    ...
    let blocked = []
    try { blocked = JSON.parse(localStorage.getItem("catchCameraBlockedWideLens") || "[]") } catch (_) {}
    ...
    const info = {
      ...
      blockedWideLensCount: blocked.length,
    ...
    if (!el) {
      ...
      copyBtn.style.cssText = "background:#222;color:#fff;border:1px solid #444;padding:4px 8px;font:12px sans-serif;border-radius:4px;cursor:pointer;margin-bottom:6px;margin-right:6px"
      ...
      resetBtn.textContent = "Clear blocklist"
      resetBtn.style.cssText = "background:#222;color:#fff;border:1px solid #444;padding:4px 8px;font:12px sans-serif;border-radius:4px;cursor:pointer;margin-bottom:6px"
      resetBtn.addEventListener("click", (event) => {
        event.stopPropagation()
        try {
          localStorage.removeItem("catchCameraBlockedWideLens")
          localStorage.removeItem("catchCameraZoom")
          resetBtn.textContent = "Cleared — reload"
        } catch (_) {
          resetBtn.textContent = "Clear failed"
        }
      })
