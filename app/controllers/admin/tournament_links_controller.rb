class Admin::TournamentLinksController < Admin::BaseController
  before_action :load_tournament

  def create
    other = current_club.tournaments.find_by(id: params[:linked_tournament_id])
    if other.nil?
      redirect_to edit_admin_tournament_path(@tournament), alert: "Pick a tournament to link with." and return
    end
    unless @tournament.mode_team? && other.mode_team?
      redirect_to edit_admin_tournament_path(@tournament), alert: "Only team tournaments can be linked." and return
    end
  ...
  def destroy
    ...
    redirect_to edit_admin_tournament_path(@tournament), notice: "Unlinked."
  end
