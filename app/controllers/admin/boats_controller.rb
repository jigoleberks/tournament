class Admin::BoatsController < Admin::BaseController
  ...
  def create
    ...
    if (match = ::Boats::NearMatch.call(club: current_club, name: name))
      if tournament.tournament_entries.exists?(boat_id: match.id)
        redirect_to edit_admin_tournament_path(tournament), alert: "#{match.name} is already entered." and return
      end
  ...
  def restore
    ...
    redirect_to admin_boats_path, notice: "#{boat.name} restored."
  end
  ...
  def purge
    ...
    if boat.active?
      ...
      name = boat.name
      boat.destroy!
      redirect_to admin_boats_path, notice: "#{name} permanently deleted."
    end
  end
