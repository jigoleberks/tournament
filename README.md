...
That's it — no Ruby, Postgres, or image tooling on the host. To rebrand, drop a square 512×512 image at `public/icon.png` (or `public/icon.jpg`). The four sized PNGs the manifest and `apple-touch-icon` reference (`public/icons/icon-{180,192,512,maskable-512}.png`) are generated from it automatically by `bin/icons` — `bin/update` calls it, and it's idempotent if the source hasn't changed. Without a source image the install still works but the manifest entries 404 and iOS shows a generic icon.
